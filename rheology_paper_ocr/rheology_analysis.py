...
def _ordered_points(series: DigitizedSeries) -> list[tuple[float, float]]:
    ...
    discarded merely because a separate log-log slope calculation cannot use it.
    """
    return sorted(
...
def series_quality_warnings(series: DigitizedSeries, x_axis_label: str | None = None) -> list[str]:
    ...
    if len(valid_points) != len(series.points):
        warnings.append("negative-x or non-positive-y digitized points were excluded from rheology analysis")
...
def summarize_series(series: DigitizedSeries, x_axis_label: str | None = None) -> SeriesSummary:
    ...
    log_points = [(x, y) for x, y in points if x > 0]
    if len(log_points) >= 2:
        first_log_x, first_log_y = log_points[0]
        last_log_x, last_log_y = log_points[-1]
        denominator = math.log10(last_log_x) - math.log10(first_log_x)
        if denominator:
            summary.loglog_slope = (math.log10(last_log_y) - math.log10(first_log_y)) / denominator
    return summary
